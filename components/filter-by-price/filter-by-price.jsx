  ...
  return (
    <>
      <div className={correct ? 'filter-by-price' : 'filter-by-price error'}>
        <input
          className={
            correctMinPrice ? 'min-price-field' : 'min-price-field error'
          }
          type="text"
        ...
        <input
          className={
            correctMaxPrice ? 'max-price-field' : 'max-price-field error'
          }
          type="text"
      ...
      <p className={correct ? 'error-text' : 'error-text visible'}>
        Пожалуйста, введите корректное значение (только цифры)
      </p>
    </>
  )
}
